...
deaths_nation = spark.sql("select dt.DIM_DATE_ID, dl.DIM_LOCATION_ID, stg_deaths.newDeaths, null as firstDose, null as secondDose from mart.dim_date dt join staging.cov_deaths_nation stg_deaths on dt.dt = stg_deaths.reported_date join mart.DIM_LOCATION dl on dl.code = stg_deaths.areacode ")

deaths_yorkshire = spark.sql("select dt.DIM_DATE_ID, dl.DIM_LOCATION_ID, stg_deaths.newDeaths, null as firstDose, null as secondDose from mart.dim_date dt join staging.cov_deaths_yorkshire stg_deaths on dt.dt = stg_deaths.reported_date join mart.DIM_LOCATION dl on dl.code = stg_deaths.areacode ")
...
doses_nation = spark.sql("select dt.DIM_DATE_ID, dl.DIM_LOCATION_ID, null as newDeaths, stg_doses.firstDose, stg_doses.secondDose from mart.dim_date dt join staging.cov_vaccinations stg_doses on dt.dt = stg_doses.reported_date join mart.DIM_LOCATION dl on dl.code = stg_doses.areacode")

...
spark = SparkSession.builder.master("yarn-client").enableHiveSupport() \
                    .appName('ADMP-Grp14-Mart-Dimensions') \
                    .getOrCreate()
...
dim_date = spark.sql("SELECT ROW_NUMBER() OVER(ORDER BY date) as id, date as dt, day, month, quarter, year from dim_date_temp")
...
infection_type_list = [(1, "Viral", "Influenza"), (2, "Viral", "Covid-19")]
...
dim_location = spark.sql("SELECT ROW_NUMBER() OVER (ORDER BY CODE DESC) as id, name, type, code from loc_temp")

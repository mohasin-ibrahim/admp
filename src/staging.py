...
spark = SparkSession.builder.master("yarn-client").enableHiveSupport() \
                    .appName('ADMP-Grp14-Covid-Staging') \
                    .getOrCreate()
...
print("FluDeaths dataset schema")
flu_stg.printSchema()
...
UNION ALL \
SELECT 'SOURCING' AS LAYER, 'HDFS FILES' AS TYPE, 'Fludeaths' AS NAME, COUNT(*) as cnt FROM flu_stg GROUP BY 'SOURCING', 'HDFS FILES', 'Fludeaths' \
UNION ALL \
...
UNION ALL \
SELECT 'STAGING' AS LAYER, 'HIVE TABLE' AS TYPE, 'Fludeaths' AS NAME, COUNT(*) as cnt FROM staging.fludeaths GROUP BY 'STAGING', 'HIVE TABLE', 'Fludeaths' \
UNION ALL \

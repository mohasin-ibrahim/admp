...
Create a directory named postgres in Documents folder and copy the path
docker run --name postgres-mart -d --rm -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres -p 5432:5432 --net cda -v C:\Users\LabStudent-55-706949\Documents\postgres:/var/lib/postgresql/data postgres:10-alpine
-----------
...
CREATE EXTERNAL TABLE IF NOT EXISTS staging.fludeaths(
    YEAR STRING,
    COUNTRY STRING,
    DEATHS STRING)
ROW FORMAT DELIMITED
...
CREATE EXTERNAL TABLE IF NOT EXISTS staging.cov_deaths_nation(
    AREACODE STRING,
    AREANAME STRING,
    AREATYPE STRING,
	REPORTED_DATE DATE,
...
CREATE EXTERNAL TABLE IF NOT EXISTS staging.cov_deaths_yorkshire(
    AREACODE STRING,
    AREANAME STRING,
    AREATYPE STRING,
	REPORTED_DATE DATE,
